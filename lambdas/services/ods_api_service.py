...
class OdsApiService:
    ...
    def fetch_organisation_with_permitted_role(self, ods_code: str) -> Dict:
        ...
        itoc_ods_codes = self.token_handler_ssm_service.get_itoc_ods_codes()
        ...
        if ods_code in itoc_ods_codes:
            ...
            return self._parse_ods_response(itoc_org_data, "", "ITOC")

        org_data = self.fetch_organisation_data(ods_code)

        logger.info(f"Org Data: {org_data}")

        gp_org_role_code = self._get_user_gp_org_role_code(org_data)

        if gp_org_role_code is not None:
            logger.info(f"ODS code {ods_code} is a GP, returning org data")
            icb_ods_code = self._find_icb_for_user(org_data["Organisation"])
            response = self._parse_ods_response(
                org_data, gp_org_role_code, icb_ods_code
            )
            return response

        pcse_ods_code = self.token_handler_ssm_service.get_pcse_ods_code()

        if ods_code == pcse_ods_code:
            logger.info(f"ODS code {ods_code} is PCSE, returning org data")
            response = self._parse_ods_response(org_data, "", "PCSE")
            return response

        allowed_ods_code_list = (
            self.token_handler_ssm_service.get_allowed_list_of_ods_codes()
        )

        if ods_code in allowed_ods_code_list:
            logger.info(f"ODS code {ods_code} is in allowed list, returning org data")
            icb_ods_code = self._find_icb_for_user(org_data["Organisation"])
            primary_org_role_code = self._get_user_primary_org_role_code(org_data)
            response = self._parse_ods_response(
                org_data, primary_org_role_code, icb_ods_code
            )
            return response
    ...
    @staticmethod
    def _parse_ods_response(org_data, role_code, icb_ods_code) -> dict:
    ...
    def _get_user_gp_org_role_code(self, org_data: Dict[str, Any]) -> Optional[str]:
        logger.info("starting ssm request to retrieve GP organisation role code")
        gp_org_role_code = self.ssm_service.get_ssm_parameter(GP_ORG_ROLE_CODE)
    ...
    @staticmethod
    def _get_user_primary_org_role_code(org_data: Dict[str, Any]) -> str:
    ...
    @staticmethod
    def _find_icb_for_user(org_data: Dict[str, Any]) -> str:
