...
def test_handle_sqs_message_happy_path(
        set_env, mocker, mock_uuid, service_under_test, mock_validate_files
):
    ...
    mock_report_upload_complete = mocker.patch.object(
        service_under_test.dynamo_repository, "report_upload_complete"
    )
    mock_remove_ingested_file_from_source_bucket = mocker.patch.object(
        service_under_test.s3_repository, "remove_ingested_file_from_source_bucket"
    )
    mocker.patch.object(
        service_under_test.s3_repository, "check_virus_result"
    )

    service_under_test.handle_sqs_message(message=TEST_SQS_MESSAGE)
...
def test_handle_sqs_message_happy_path_with_non_ascii_filenames(
    ...
    set_up_mocks_for_non_ascii_files(service_under_test, mocker, patient_name_on_s3)
    ...
    service_under_test.dynamo_repository.report_upload_complete.assert_called()
    assert service_under_test.s3_repository.check_virus_result.call_count == 1
    assert service_under_test.s3_repository.copy_to_lg_bucket.call_count == 3


def test_handle_sqs_message_calls_report_upload_failure_when_patient_record_already_in_repo(
        service_under_test, set_env, mocker, mock_uuid, mock_validate_files
):
    mock_create_lg_records_and_copy_files = mocker.patch.object(
        BulkUploadService, "create_lg_records_and_copy_files"
    )
    ...
    mock_report_upload_failure = mocker.patch.object(
        service_under_test.dynamo_repository, "report_upload_failure"
    )
...
def test_handle_sqs_message_calls_report_upload_failure_when_lg_file_name_invalid(
    ...
    service_under_test.handle_sqs_message(message=TEST_SQS_MESSAGE_WITH_INVALID_FILENAME)
...
def test_handle_sqs_message_report_failure_when_document_is_infected(
        service_under_test, set_env, mocker, mock_uuid, mock_validate_files, mock_check_virus_result
):
    mock_report_upload_failure = mocker.patch.object(
        service_under_test.dynamo_repository, "report_upload_failure"
    )
    ...
    service_under_test.s3_repository.check_virus_result.side_effect = DocumentInfectedException
...
def test_handle_sqs_message_report_failure_when_document_not_exist(
    ...
    service_under_test.s3_repository.check_virus_result.side_effect = S3FileNotFoundException
    mock_report_upload_failure = mocker.patch.object(
        service_under_test.dynamo_repository, "report_upload_failure"
    )
...
def test_handle_sqs_message_put_staging_metadata_back_to_queue_when_virus_scan_result_not_available(
    ...
    service_under_test.s3_repository.check_virus_result.side_effect = VirusScanNoResultException
    mock_report_upload_failure = mocker.patch.object(
        service_under_test.dynamo_repository, "report_upload_failure"
    )
    mock_create_lg_records_and_copy_files = mocker.patch.object(
    ...
    mock_put_staging_metadata_back_to_queue = mocker.patch.object(
        service_under_test.sqs_repository, "put_staging_metadata_back_to_queue"
    )
...
def test_handle_sqs_message_rollback_transaction_when_validation_pass_but_file_transfer_failed_halfway(
        service_under_test, set_env, mocker, mock_uuid, mock_check_virus_result, mock_validate_files
):
    mock_rollback_transaction_s3 = mocker.patch.object(
        service_under_test.s3_repository, "rollback_transaction"
    )
    mock_rollback_transaction_dynamo = mocker.patch.object(
        service_under_test.dynamo_repository, "rollback_transaction"
    )
    mock_report_upload_failure = mocker.patch.object(
        service_under_test.dynamo_repository, "report_upload_failure"
    )
    ...
    service_under_test.s3_repository.copy_to_lg_bucket.side_effect = [None, None, mock_client_error]
...
def test_handle_sqs_message_raise_InvalidMessageException_when_failed_to_extract_data_from_message(
        service_under_test, set_env, mocker
):
    ...
    with pytest.raises(InvalidMessageException):
        service_under_test.handle_sqs_message(invalid_message)

    mock_create_lg_records_and_copy_files.assert_not_called()


def test_validate_files_raise_LGInvalidFilesException_when_file_names_invalid(
        service_under_test, set_env, mock_validate_files
):
    ...
    service_under_test.handle_sqs_message({"body": invalid_file_name_metadata_as_json})
    service_under_test.dynamo_repository.report_upload_failure.assert_called()


@freeze_time("2023-10-2 13:00:00")
def test_reports_failure_when_max_retries_reached(set_env, mocker, mock_uuid, service_under_test, mock_validate_files):
    mock_failure_reason = (
    ...
    mock_validate_files.side_effect = LGInvalidFilesException
    service_under_test.handle_sqs_message({"body": metadata_as_json})

    service_under_test.sqs_repository.send_message_with_nhs_number_attr_fifo.assert_not_called()
    service_under_test.dynamo_repository.report_upload_failure.assert_called()


def test_resolve_source_file_path_when_filenames_dont_have_accented_chars(set_env, service_under_test):
    expected = {
    ...
    actual = service_under_test.file_path_cache
...
def test_resolve_source_file_path_when_filenames_have_accented_chars(
        set_env, mocker, patient_name_on_s3, patient_name_in_metadata_file, service_under_test
):
    expected_cache = {}
    for i in range(1, 4):
...
def test_resolves_source_file_path_raise_S3FileNotFoundException_if_filename_cant_match(
    ...
    service_under_test.s3_repository.file_exists_on_staging_bucket.return_value = False
...
def test_create_lg_records_and_copy_files(set_env, mocker, mock_uuid, service_under_test):
    ...
    for file in TEST_STAGING_METADATA.files:
        ...
        service_under_test.s3_repository.copy_to_lg_bucket.assert_any_call(
            source_file_key=expected_source_file_key,
            dest_file_key=expected_dest_file_key,
        )
    assert service_under_test.s3_repository.copy_to_lg_bucket.call_count == 3

    service_under_test.dynamo_repository.create_record_in_lg_dynamo_table.assert_any_call(
        TEST_DOCUMENT_REFERENCE
    )
    assert service_under_test.dynamo_repository.create_record_in_lg_dynamo_table.call_count == 3


def test_convert_to_document_reference(set_env, mock_uuid, service_under_test):
    service_under_test.s3_repository.lg_bucket_name = 'test_lg_s3_bucket'
    expected = TEST_DOCUMENT_REFERENCE
    actual = service_under_test.convert_to_document_reference(
