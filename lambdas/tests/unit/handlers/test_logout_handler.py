...
def test_logout_handler_valid_jwt_returns_200(mocker, monkeypatch, context):
...
def test_logout_handler_invalid_jwt_returns_400(mocker, monkeypatch, context):
    mock_token = "mock_token"
    monkeypatch.setenv("SSM_PARAM_JWT_TOKEN_PUBLIC_KEY", "mock_public_key")
...
def test_logout_handler_boto_error_returns_500(mocker, monkeypatch, context):
    mock_token = "mock_token"
    mock_session_id = "mock_session_id"
    mock_decoded_token = {"ndr_session_id": mock_session_id}
    monkeypatch.setenv("SSM_PARAM_JWT_TOKEN_PUBLIC_KEY", "mock_public_key")
...
def build_event_from_token(token: str) -> dict:
    return {"headers": {"x-auth": token, "Authorization": token}}
