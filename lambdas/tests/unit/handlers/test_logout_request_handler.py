...
@pytest.fixture
def mock_get_public_key(mocker, monkeypatch):
    monkeypatch.setenv("SSM_PARAM_JWT_TOKEN_PUBLIC_KEY", "mock_public_key_secret_name")
    mocker.patch("handlers.logout_handler.get_secret", return_value="mock_public_key")
    yield


def test_logout_handler_valid_jwt_returns_200(mocker, mock_get_public_key):
...
def test_logout_handler_invalid_jwt_returns_400(mocker, mock_get_public_key):
    mock_token = "mock_token"
...
def test_logout_handler_jwt_without_ndr_session_id_returns_400(mocker, mock_get_public_key):
    mock_token = "mock_token"
...
def test_logout_handler_boto_error_returns_500(mocker, mock_get_public_key):
    mock_token = "mock_token"
    mock_session_id = "mock_session_id"
    mock_decoded_token = {"ndr_session_id": mock_session_id}
    mock_token_validator = mocker.patch("jwt.decode", return_value=mock_decoded_token)
...
def build_event_from_token(token: str) -> dict:
    return {"headers": {"authorizationToken": token}}
