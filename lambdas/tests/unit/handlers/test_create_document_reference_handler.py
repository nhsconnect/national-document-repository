...
def test_creates_document_reference_and_returns_presigned_url(invalid_event, context, mocker):
    ...
    with patch.object(DynamoReferenceService, "create_document_dynamo_reference_object", return_value=mock_doc_ref):
        with patch.object(S3UploadService, "create_document_presigned_url_handler",
                          return_value=MOCK_PRESIGNED_POST_RESPONSE):
            with patch.object(DynamoReferenceService, "save_document_reference_in_dynamo_db"):
