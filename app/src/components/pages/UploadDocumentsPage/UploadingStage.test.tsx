...
describe("<UploadDocumentsPage />", () => {
    describe("with NHS number", () => {
        const triggerUploadStateChange = (document: UploadDocument, state : DOCUMENT_UPLOAD_STATE, progress: number) => {
            act(() => {
        ...
        it("progress bar reflect the upload progress", async () => {
            ...
            const getProgressBarValue = (document: UploadDocument) => {
                const progressBar: HTMLProgressElement = screen.getByRole("progressbar", { name: `Uploading ${document.file.name}` });
