...
describe("<UploadDocumentsPage />", () => {
  describe("upload documents with an NHS number", () => {
    ...
    it("renders the page", () => {
      ...
      expect(
        screen.getByText(mockPatientDetails.nhsNumber),
      ).toBeInTheDocument();
    ...
    it("renders link to PCSE that opens in a new tab", () => {
      ...
      expect(pcseLink).toHaveAttribute(
        "href",
        "https://secure.pcse.england.nhs.uk/",
      );
...
const renderSelectStage = (setDocumentMock: jest.Mock) => {
  render(
    <SelectStage setDocuments={setDocumentMock} uploadDocuments={() => {}} />,
  );
};
