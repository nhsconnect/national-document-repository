...
describe('UploadSummary', () => {
  it('renders the page', () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Upload Summary' })
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.queryByText('View successfully uploaded documents')
    ).not.toBeInTheDocument();
  });
  ...
  it('displays successfully uploaded docs and callout message', () => {
    ...
    expect(
      screen.getByText('View successfully uploaded documents')
    ).toBeInTheDocument();
    ...
    files.forEach(({ name, size }) => {
      ...
      expect(
        within(uploadedDocsTable).getByText(formatSize(size))
      ).toBeInTheDocument();
    });
    expect(screen.getByText('Before you close this page')).toBeInTheDocument();
  });
  ...
  it('does not include docs that failed to upload in the successfully uploaded docs list', () => {
    ...
    const documents = [
      buildDocument(
        buildTextFile(uploadedFileName, 100),
        documentUploadStates.SUCCEEDED
      ),
      buildDocument(
        buildTextFile(failedToUploadFileName, 101),
    ...
    expect(
      within(uploadedDocsTable).getByText(`${uploadedFileName}.txt`)
    ).toBeInTheDocument();
    expect(
  ...
  it('does not display the successfully uploads docs list when all of the docs failed to upload', () => {
    const documents = [
      buildDocument(buildTextFile('test1'), documentUploadStates.FAILED)
    ];
    ...
    expect(
      screen.queryByText('View succe0ssfully uploaded documents')
    ).not.toBeInTheDocument();
  });
  ...
  it('displays message and does not display an alert if all the docs were uploaded successfully', () => {
    ...
    expect(
      screen.queryByText(
        `${documents.length} of ${files.length} files failed to upload`
      )
    ).not.toBeInTheDocument();
  });
  ...
  it('displays an alert if some of the docs failed to upload', () => {
    ...
    expect(
      screen.getAllByText('Documents that have failed to upload')
    ).toHaveLength(2);
    expect(
  ...
  it('displays each doc that failed to upload in a table', () => {
    ...
    files.forEach(({ name, size }) => {
      expect(
        within(failedToUploadDocsTable).getByText(name)
      ).toBeInTheDocument();
      expect(
        within(failedToUploadDocsTable).getByText(formatSize(size))
      ).toBeInTheDocument();
    });
  });
...
const renderUploadSummary = (propsOverride: Props) => {
  const props = {
    ...propsOverride,
    documents: propsOverride.documents ?? []
  };
