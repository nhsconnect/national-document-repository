describe('Uploads docs and tests it looks OK', () => {
    ...
    it('Multiple files - On Upload button click, renders Upload Summary for successful upload', () => {
        ...
        cy.get("#upload-summary-header").should('be.visible')
        cy.get('#successful-uploads').should('be.visible')
        cy.get("#successful-uploads tbody tr").should('have.length', 11)
        cy.get("#successful-uploads tbody tr").first().get('td').first().should('contain', 'test_image.jpg')
        cy.get('#close-page-warning').should('be.visible')
        cy.get('#start-again-button').should('have.text', 'Start Again')
        cy.get('#start-again-button').click()
        cy.url().should('eq', localhost)
    ...
    it('Multiple files - On Upload button click, renders Uploading Stage', () => {
     ...
     it('Multiple files - On Upload button click, renders Upload Summary with error box when DocumentReference returns a 500', () => {
        // intercept this response and return an error
        cy.intercept('POST', '*/DocumentReference*', {
            statusCode: 500,
        })
    ...
    it('Multiple files - On Upload button click, renders Upload Summary with error box when DocumentReference returns a 404', () => {
        // intercept this response and return an error
        cy.intercept('POST', '*/DocumentReference*', {
            statusCode: 404,
        })
    ...
    // once authorisation functionality has been added we can include a test like below for s3 bucket POST failure with 403 status
    // it('Multiple file - On Upload button click, renders Upload Summary with error box when the s3 bucket POST request fails', () => {
    //     // intercept this response and return an error
